...
func (r Runner) runInteractiveCommand(ctx context.Context, env map[string]string, name string, args ...string) error {
	path, err := exec.LookPath(name)
	if err != nil {
		return fmt.Errorf("find %s: %w", name, err)
	}
...
func (r Runner) runOutputCommandWithExitCode(ctx context.Context, env map[string]string, name string, args ...string) (string, int, error) {
	path, err := exec.LookPath(name)
	if err != nil {
	...
	return string(out), 0, fmt.Errorf("%s %s: %w\n%s", name, strings.Join(args, " "), err, trimmed)
}
