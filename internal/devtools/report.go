...
func (r Runner) Report(opts ReportOptions) error {
	...
	if err := cleanr.WriteReport(&buf, report, format); err != nil {
		return err
	}
	path := resolvePath(r.WorkDir, output)
	if err := os.MkdirAll(filepath.Dir(path), 0o755); err != nil {
		return fmt.Errorf("prepare report output: %w", err)
	}
	if err := os.WriteFile(path, buf.Bytes(), 0o644); err != nil {
		return fmt.Errorf("write report output: %w", err)
	}
	_, err = fmt.Fprintf(r.Stdout, "rendered %s report from %s to %s\n", format, source, path)
	return err
}
...
func demoReport(preset string) (cleanr.Report, error) {
	...
	case "pass":
		return demoPassReport(generated), nil
	case "", "fail":
		return demoFailReport(generated), nil
	default:
		return cleanr.Report{}, fmt.Errorf("unsupported report preset: %s", preset)
	}
}

func demoPassReport(generated time.Time) cleanr.Report {
...
func demoFailReport(generated time.Time) cleanr.Report {
	return cleanr.Report{
		Name:            "demo-assistant",
		Passed:          false,
		GeneratedAt:     generated,
