...
The active `codeguard` policy in this repository is defined in [.codeguard/codeguard.yaml](../.codeguard/codeguard.yaml) and currently enables quality, security, and CI checks with a `400` max-file-lines threshold and a `20` max-cyclomatic-complexity threshold.
